...
std::string Agent::buildSystemPrompt() const {
    ...
    if (prompt.empty()) {
        ...
        "2. Query llm_calls for the run_id and node_name='deep_analysis' to read the LLM analysis text\n"
        "3. Use CALLGRAPH or XREFS to trace how suspicious code connects\n"
        "4. Only then produce your final ANSWER with concrete evidence\n\n"

        "=== CRITICAL: DO NOT PROBE THE LOCAL FILESYSTEM FOR DATABASE PATHS ===\n"
        "File paths in scan_results, llm_calls, and method_findings are from the ORIGINAL "
        "machine where the scan ran. These files do NOT exist on your filesystem. NEVER use "
        "FIND_FILES, READ, DISASM, DECOMPILE, ls, or any filesystem tool to access paths you "
        "found in database query results. Instead, use SQL queries on llm_calls (which stores "
        "the full prompt and response text for every analysis step) to retrieve the actual "
        "analysis content. Use XREFS and CALLGRAPH which are database-backed.\n\n"
