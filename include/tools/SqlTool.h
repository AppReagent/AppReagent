...
public:
    ...
    std::string description() const override {
        return "<query> — query the PostgreSQL database. Key tables:\n"
               ...
               "  method_findings (run_id, file_path, class_name, method_name, api_calls, findings, reasoning, relevant, confidence)\n"
               "  method_calls (run_id, caller_class, caller_method, callee_class, callee_method, invoke_type)\n"
               "  llm_calls (run_id, file_path, node_name, prompt, response, latency_ms)\n"
               "  Example: SQL: SELECT file_path, risk_score, recommendation FROM scan_results ORDER BY run_id DESC LIMIT 5";
    }
